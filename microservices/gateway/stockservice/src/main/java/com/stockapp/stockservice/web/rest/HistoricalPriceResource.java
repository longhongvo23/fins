...
/**
 * REST controller for managing {@link com.stockapp.stockservice.domain.HistoricalPrice}.
 */
@RestController
...
public class HistoricalPriceResource {
    ...
    public HistoricalPriceResource(HistoricalPriceService historicalPriceService, HistoricalPriceRepository historicalPriceRepository) {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new historicalPriceDTO, or with status {@code 400 (Bad Request)} if the historicalPrice has already an ID.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("")
    public Mono<ResponseEntity<HistoricalPriceDTO>> createHistoricalPrice(@Valid @RequestBody HistoricalPriceDTO historicalPriceDTO)
        throws URISyntaxException {
        LOG.debug("REST request to save HistoricalPrice : {}", historicalPriceDTO);
        if (historicalPriceDTO.getId() != null) {
            throw new BadRequestAlertException("A new historicalPrice cannot already have an ID", ENTITY_NAME, "idexists");
        }
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated historicalPriceDTO,
     * or with status {@code 400 (Bad Request)} if the historicalPriceDTO is not valid,
     * or with status {@code 500 (Internal Server Error)} if the historicalPriceDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/{id}")
    ...
    /**
     * {@code PATCH  /historical-prices/:id} : Partial updates given fields of an existing historicalPrice, field will ignore if it is null
     *
     * @param id the id of the historicalPriceDTO to save.
     * @param historicalPriceDTO the historicalPriceDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated historicalPriceDTO,
     * or with status {@code 400 (Bad Request)} if the historicalPriceDTO is not valid,
     * or with status {@code 404 (Not Found)} if the historicalPriceDTO is not found,
     * or with status {@code 500 (Internal Server Error)} if the historicalPriceDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    ...
    public Mono<ResponseEntity<HistoricalPriceDTO>> partialUpdateHistoricalPrice(
        @PathVariable(value = "id", required = false) final String id,
        @NotNull @RequestBody HistoricalPriceDTO historicalPriceDTO
    ) throws URISyntaxException {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of historicalPrices in body.
     */
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the historicalPriceDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/{id}")
